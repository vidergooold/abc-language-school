...
async function submitChild() {
  try {
    ...
    Object.keys(childForm.value).forEach(key => {
      if (typeof childForm.value[key] === 'boolean') {
        childForm.value[key] = false
      } else {
        childForm.value[key] = ''
      }
    })
  } catch (error) {
    alert('Ошибка отправки анкеты. Попробуйте позже.')
    console.error(error)
  }
}
...
async function submitAdult() {
  try {
    ...
    Object.keys(adultForm.value).forEach(key => {
      adultForm.value[key] = ''
    })
  } catch (error) {
    alert('Ошибка отправки анкеты. Попробуйте позже.')
...
async function submitPreschool() {
  try {
    ...
    Object.keys(preschoolForm.value).forEach(key => {
      preschoolForm.value[key] = ''
    })
  } catch (error) {
    alert('Ошибка отправки анкеты. Попробуйте позже.')
    console.error(error)
