<template>
  <div>
    ...
    <p v-if="!enrollmentsStore.loading && !enrollmentsStore.enrollments.length">
...
const email = computed(() => {
  // пока у нас в токене только id, email можно будет получать
  // через отдельный /auth/me; пока показываем «—»
  return '—'
})
