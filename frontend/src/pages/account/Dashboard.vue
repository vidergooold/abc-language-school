...
onMounted(async () => {
  ...
  try {
    ...
    stats.value = [
      { icon: '📝', label: 'Анкет за всё время', value: s.total_forms ?? '—' },
      { icon: '👥', label: 'Учеников', value: s.total_students ?? '—' },
      { icon: '💬', label: 'Обращений', value: s.total_feedback ?? '—' },
      { icon: '🗓', label: 'Групп в расписании', value: s.schedule_groups ?? '—' },
    ]
  } catch { /* silent */ }
})
</script>
